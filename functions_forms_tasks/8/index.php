...
//check for obscene
function ifObscene($str){
	//так как цель не указана, буду считать, что нужно не потерять лицензию СМИ РФ
	//http://www.interfax.ru/russia/348755
	//список сократил до примитивного
	$bads = [
		"хуй",
		"пизда",
		"шлюха",
	...
	$str = trim($str);
	$pattern = "/[\s-_!,.:+=?]+/";
	$stringArr = preg_split($pattern, $str);

	return !empty(array_uintersect($bads, $stringArr, "strcasecmp"));
}
...
if(isRequestPost()) {
	if(isFormNotEmpty()) {
		if (!ifObscene(trim($_POST['userComment']))) {
			$newComment = array(
				'id' => uniqid(),
				'userName' => trim($_POST['userName']),
				'userComment' => strip_tags(trim($_POST['userComment']), "<b>")
				);
			...
			$errorMessage = "Offensive detected";
		}
	} else {
